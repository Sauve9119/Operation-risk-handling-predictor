...
def train_pipeline(_df):
    ...
    # Pick best by F1, accuracy as tiebreaker
    best_name = max(results, key=lambda n: (results[n]['F1 Score'], results[n]['Accuracy']))

    # Refit best model cleanly on full training set
...
# ═══════════════════════════════════════════════════════════════
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("### 🔀 K-Fold Cross-Validation Split Visualization")
    # Show K-Fold on the actual training data that was used for CV
    skf_v  = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    X_d    = np.zeros((len(y_tr), 1))
    fig, ax = plt.subplots(figsize=(12,3.5)); fig.patch.set_facecolor('#0d1525'); ax.set_facecolor('#0d1525')
    for i, (_, ti) in enumerate(skf_v.split(X_d, y_tr)):
        fa = np.zeros(len(y_tr)); fa[ti] = 1
    ...
    ax.set_xlabel("Training Sample Index", color='#6a8db8')
    ax.set_title("K-Fold Split on Training Data  (🔴 Validation  |  🔵 Train)", color='#e8f0fe', fontsize=12, pad=12)
