...
def load_and_prepare():
    ...
    mappings = [
        ...
        {"Miss some tasks":1,"Get confused":2,"Handle with difficulty":3,"Adjust priorities":4,"Confidently re-prioritize":5},
...
def train_pipeline(_df):
    ...
    results = {}
    best_name, best_mdl, best_f1 = None, None, 0
    for name, mdl in mdls.items():
        mdl.fit(X_tr, y_tr)
        sc = cross_validate(mdl, X_tr, y_tr, cv=skf, scoring={'accuracy':'accuracy','precision':'precision_weighted','recall':'recall_weighted','f1':'f1_weighted'})
        results[name] = {k.replace('test_','').title(): round(v.mean(), 4) for k, v in sc.items() if 'test_' in k}
        # fix key names
        results[name] = {'Accuracy': round(sc['test_accuracy'].mean(),4), 'Precision': round(sc['test_precision'].mean(),4),
                         'Recall': round(sc['test_recall'].mean(),4), 'F1 Score': round(sc['test_f1'].mean(),4)}
        if results[name]['F1 Score'] > best_f1:
            best_f1 = results[name]['F1 Score']
            best_name, best_mdl = name, mdl
...
# ═══════════════════════════════════════════════════════════════
    ...
    with tab3:
        ...
        ax.set_xlabel('Number of Clusters', color='#6a8db8'); ax.set_ylabel('BIC Score', color='#6a8db8')
    ...
    with tab4:
        ...
        for i, (cn, ax) in enumerate(zip(columns, axes.flatten())):
...
# ═══════════════════════════════════════════════════════════════
    ...
    cc = [['#0d1525' if j==0 else ('#0f3d1f' if float(v)>=0.8 else '#1a2f0f' if float(v)>=0.6 else '#2a1515') for j,v in enumerate(row)] for row in res_df.values.tolist()]
    ...
    st.markdown("<hr>### 🔀 K-Fold Visualization", unsafe_allow_html=True)
    ...
    X_d = np.zeros((len(y_full),1))
    for i, (_, ti) in enumerate(skf_v.split(X_d, y_full)):
    ...
    ax.set_title("K-Fold Split  (🔴 Test  |  🔵 Train)", color='#e8f0fe', fontsize=12, pad=12)
    ax.tick_params(colors='#4a6a9a'); ax.spines[:].set_color('#1e2d4a')
    plt.tight_layout(); st.pyplot(fig)
...
# ═══════════════════════════════════════════════════════════════
    ...
    if clicked:
        ...
        predicted = best_model[1].predict(inp)[0]
        ...
        cl_avg = df_tmp.groupby('cluster')[columns].mean().loc[predicted].values

        fig, ax = plt.subplots(figsize=(9,5)); fig.patch.set_facecolor('#0d1525'); ax.set_facecolor('#0d1525')
        xp = np.arange(len(columns))
        ...
        ax.set_xticks(xp); ax.set_xticklabels([c[:10] for c in columns], rotation=25, ha='right', color='#6a8db8', fontsize=9)
